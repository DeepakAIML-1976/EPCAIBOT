...
def save_vendor(name, datasheet_id, content):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO vendors (name, datasheet_id, content) VALUES (?, ?, ?)",
...
def get_vendors(datasheet_id=None):
    conn = get_connection()
    cur = conn.cursor()
    if datasheet_id:
        cur.execute("SELECT * FROM vendors WHERE datasheet_id = ?", (datasheet_id,))
    else:
...
def save_tq(datasheet_id, vendor_id, content, category="General", approved=False):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO tqs (datasheet_id, vendor_id, content, category, approved) VALUES (?, ?, ?, ?, ?)",
        (datasheet_id, vendor_id, content, category, int(approved)),
    )
    conn.commit()
    tq_id = cur.lastrowid
    conn.close()
...
def get_tqs(datasheet_id=None, only_approved=False):
    conn = get_connection()
    cur = conn.cursor()
    query = "SELECT * FROM tqs"
    params = []
    if datasheet_id and only_approved:
        query += " WHERE datasheet_id = ? AND approved = 1"
        params = [datasheet_id]
    elif datasheet_id:
        query += " WHERE datasheet_id = ?"
        params = [datasheet_id]
    elif only_approved:
...
def save_tbe(datasheet_id, datasheet_name, html_content):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO tbes (datasheet_id, datasheet_name, html_content) VALUES (?, ?, ?)",
